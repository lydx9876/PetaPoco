...
namespace PetaPoco.Extensions.String
{
    ...
    public class STR
    {
        ...
        {
            if (value == null || value.Length == 0) return this;
            this.__SB.Insert(index, string.Concat(value));
            return this;
        }
        ...
        private STR TrimHelper(char[] trimChars, int trimType)
        {
            ...
            bool f = trimChars == null || trimChars.Length == 0;
            int index;
            if (trimType != 1)
            {
                for (index = 0; index < this.__SB.Length; index++)
                {
                    if (f)
                    {
                        if (!(char.IsWhiteSpace(this.__SB[index]) || this.__SB[index] == char.Parse("")))
                            break;
                    }
                    else
                    {
                        if (!(trimChars.Contains(this.__SB[index])))
                            break;
                    }
                }
                if (index > 0)
