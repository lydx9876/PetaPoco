...
using PetaPoco.Extensions.String;
using System;
...
namespace PetaPoco.Extensions.Linq
{
    ...
    public class GetBuilder<T> : GetBuilder where T : new()
    {
        ...
        private string GetOperant(ExpressionType expType)
        {
            switch (expType)
            {
                ...
                case ExpressionType.NotEqual:
                    return "<>";
                default:
                    throw new NotSupportedException(string.Format("不支持 “{0}” 操作符", expType.ToString()));
            }
        }
        #endregion
        ...
        {
            if (__select == null || __select[0] == '*') __select = new STR();
            if (!__select.IsEmpty)
                __select.Append(", ");
        ...
        {
            return Select(string.Format("{0}.{1}", tableName, columnName));
        }
        ...
        public GetBuilder<T> Select(params Expression<Func<T, object>>[] expression)
        {
            foreach (var item in expression)
                this.Select(__db.GetTableAndColumnName(item));
            return this;
        }
        ...
        {
            if (__orderby == null) __orderby = new STR(" ORDER BY ");
            foreach (var item in expression)
            {
                if (__orderby.Length > 10)
                    __orderby.Append(", ");
                __orderby.Append(__db.GetTableAndColumnName(item));
                if (desc)
        ...
        {
            if (__select == null || __select.IsEmpty) return "*";
            return __select.ToString();
        }
        private string GetFrom()
        {
            if (__from == null) return __table;
            return string.Format("({0}) {1}", __from.GetSql(), __table);
        }
        private string GetJoin()
        {
            if (__join == null || __join.IsEmpty) return string.Empty;
        ...
        {
            if (__orderby == null) return string.Empty;
        ...
        internal override string GetSql()
        {
            ...
            return string.Format("SELECT {0}{1} FROM {2}{3} {4}{5}", dist, this.GetSelect(), this.GetFrom(), this.GetJoin(), this.GetWhere(), this.GetOrderBy());
        }
        ...
        {
            return __db.Provider.BuildPagedSql(__skip, __take, __dist, this.GetSelect(), this.GetFrom(), this.GetJoin(), this.GetWhere(), this.GetOrderBy(), this.Params);
        }
        #endregion
        ...
        public TResult Count<TResult>()
        {
            if (__dist)
            {
                string sql = string.Format("SELECT COUNT(*) FROM (SELECT DISTINCT {0} FROM {1}{2} {3})", this.GetSelect(), this.GetFrom(), this.GetJoin(), this.GetWhere());

                return __db.ExecuteScalar<TResult>(sql, this.GetParams());
            }
            return __db.ExecuteScalar<TResult>(string.Format("SELECT COUNT(*) FROM {0}{1} {2}"
                , this.GetFrom(), this.GetJoin(), this.GetWhere()), this.GetParams());
        }
        #endregion
        ...
        {
            return __db.ExecuteScalar<TResult>(string.Format("SELECT SUM({0}) FROM {1}{2} {3}", __db.GetColumnName(expression), this.GetFrom(), this.GetJoin(), this.GetWhere()), this.GetParams());
        }
        /// <summary>
        /// 获取指定字段的和
        /// </summary>
        ...
        {
            return __db.ExecuteScalar<TResult>(string.Format("SELECT MAX({0}) FROM {1}{2} {3}", __db.GetColumnName(expression), this.GetFrom(), this.GetJoin(), this.GetWhere()), this.GetParams());
        }
        /// <summary>
        /// 获取指定字段的最大值
        /// </summary>
        ...
        {
            return __db.ExecuteScalar<TResult>(string.Format("SELECT MIN({0}) FROM {1}{2} {3}", __db.GetColumnName(expression), this.GetFrom(), this.GetJoin(), this.GetWhere()), this.GetParams());
        }
        /// <summary>
        /// 获取指定字段的最小值
        /// </summary>
        ...
        {
            if (__hasMap)
                return __db.Query<T>(__mapType, __mapCallback, this.GetSql(), this.GetParams()).FirstOrDefault(predicate);
            __take = 1;
            return __db.Query<T>(this.GetSql(), this.GetParams()).FirstOrDefault(predicate);
        }
        ...
        public PagedList<T> ToPagedList(int pageIndex, int pageSize)
        {
            ...
            if (count > 0)
            {
                var pages = Number.GetPages_(count, pageSize);
